...
- `X-Executor` is a mandatory header on every mutating call across all services (previously inconsistent: sometimes a body field, sometimes absent entirely). The Hash Token Registry additionally requires `X-Source-Service`; `X-Tenant-Id` remains mandatory on tenant-scoped retrieval.
...
This is the reference convention for the remaining five services in the blueprint.
